...
# Dados de treinamento com buracos no meio
def get_cubic_example_with_gaps(n_low, n_high, sigma_noise):
    """
    Gera dados de treinamento com buracos no meio.
    `n_low` são os pontos concentrados nas extremidades.
    `n_high` define a densidade de pontos próximos aos máximos/mínimos.
    """
    # Amostras próximas aos extremos (-3, -2) e (2, 3)
    X_low = -3 + 0.5 * torch.rand(n_low).reshape(-1, 1)  # Região [-3, -2.5]
    X_high = 2.5 + 0.5 * torch.rand(n_high).reshape(-1, 1)  # Região [2.5, 3]
    
    # Ponto no meio para destacar o buraco
    X_mid = torch.tensor([[0.0]])

    # Combinar as regiões
    X_train = torch.cat([X_low, X_high, X_mid], dim=0)
...
# Configurar os dados com buracos
X_train, y_train, train_loader, X_test = get_cubic_example_with_gaps(
...
plt.ylabel('y')
plt.legend()
plt.title("Laplace Approximation com Marglik spaced")
plt.show()
...
# Salvar o gráfico
file_name = "grafico_marglik_spaced.png"
